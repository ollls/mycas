...
import java.util.concurrent.atomic.{AtomicMarkableReference, AtomicReference}
...
object NodeRef {
  ...
  /////////////////////////////////////////////////
  def constructEmpty[A]( ref_first: Node[A] = null, ref_last: Node[A] = null)(implicit ord: A => Ordered[A]): NodeRef[A] = {
    new FirstRef[A](new LastRef[A](ref_last), ref_first)
  }

  /////////////////////////////////////////////////
  def constructEmptyEx[A]( external_next_last : Node[A], ref_first: Node[A] = null )(implicit ord: A => Ordered[A]): NodeRef[A] = {
    new FirstRef[A]( external_next_last, ref_first)
  }
}

//has only a
class Node[A](var a: A = null.asInstanceOf[A],
              next: Node[A] )(implicit ord: A => Ordered[A])
  extends AtomicMarkableReference[Node[A]](next, false) with Ordered[Node[A]] {
...
//has a and extra ref
class NodeRef[A](a: A = null.asInstanceOf[A],
                 var ref: Node[A],
                 next: Node[A],
                 val orig: Node[A])(implicit ord: A => Ordered[A]) extends Node[A](a, next) {
...
private class FirstRef[A](next: Node[A], ref: Node[A] = null, orig : Node[A] = null )(implicit ord: A => Ordered[A]) extends NodeRef[A](null.asInstanceOf[A], ref, next, orig ) {
...
class LastRef[A](ref: Node[A] = null)(implicit ord: A => Ordered[A]) extends NodeRef[A](null.asInstanceOf[A], ref, null, null ) {
...
object OrderedList {
  ...
  def findClosestLesserValue2[A](list: Node[A], repeat: Node[A], pred2 : Array[Node[A]], a: A): Node[A] = {
    ...
    if (curr_marked(0) == true && ( pred.compareAndSet(curr, succ, false, false) == false)  ) {
        ...
        else { /*println( "findClosestLesserValue2 - cannot repeat" ); */ null }
    }
    ...
    //corner case, when empty: here pred.getReference() will be Last, last's lt() always false, we will return after pred, which was First and before Las
      ...
      findClosestLesserValue2(curr, repeat, pred2, a) }//advance one more cur and succ, preserve initial or start node
  ...
  //////////////////////////////////////////////////////////////////////////////
  @tailrec
  def insertInRange[A](start: Node[A],
                       from: Node[A],
                       to: Node[A],
                       new_n: Node[A],
                       count: Array[Int],
    ...
    if (pred.eq(to) == false ) {
      ...
      else if (curr.lt( new_n.a ) ) //counting, but we did not find a place to insert yet
      {
        ...
        } else insertInRange(start, curr, to, new_n, count, factor, newTo, added, done = true) //ignore dupe, counting with done = true
      }
    } else if (done == true) {
  ...
  def removeFromRange[A](start: Node[A],
                         from: Node[A],
                         to: Node[A],
                         a: A,
                         count: Array[Int],
                         ...
                         abort: Boolean,
                         newSplit : Array[Node[A]] )(implicit ord: A => Ordered[A]): Boolean =
      {
...
//////////////////////////////////////////////////////////////////////////////
  //we use extra abort flag, to show the difference between removal of non-existing item and failed removal
  @tailrec
  private def removeFromRange_t[A](start: Node[A],
                         from: Node[A],
                         to: Node[A],
                         a: A,
                         count: Array[Int],
