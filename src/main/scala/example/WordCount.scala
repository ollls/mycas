...
object RunMe {
  ...
  def exec[T](es: ExecutorService)(r: => T): Future[T] =
    es.submit(new Callable[T] { def call = r })
  ...
  /////////////////////////////////////////////////////////
  def file_task(
      acum: SkipList[StringIgnoreCaseValuePair[AtomicInteger]],
      fname: String,
      offset: Long,
      delta: Long,
      cl: CountDownLatch
      ...
      while (cur < offset + delta) {
        ...
        if (Character.isLetterOrDigit(bb_prev)) {
          ...
          if (str.charAt(0).isDigit == false) { // ignore anything wich starts with digit
            ...
            val opt =
              acum.get(StringIgnoreCaseValuePair(str, new AtomicInteger(0)))
            opt match {
              case None => {
                val res =
                  acum.add(StringIgnoreCaseValuePair(str, new AtomicInteger(1)))
                ...
                if (res == false) {
                  ...
                  val opt = acum.get(
                    StringIgnoreCaseValuePair(str, new AtomicInteger(0))
                  )
  ...
  def word_count(): Unit = {
    ...
    while {
      val offset =
        start_offset // copy for lazy eval in the thread, offset inside of lazy block passed to exec()
      exec(es)(file_task(SL, MY_FILE_PATH, offset, dt, endSignal))
      start_offset = start_offset + dt
      start_offset <= size - dt
    } do ()
    ...
    var counter = 0;
    SL_REV.foreach(
      c => println("%-10s %s".format(c.value, c.key)),
      c => {
        if (counter > 12) false
